...
// Function that splits the card pile into two of equal length using the splice method.
    ...
    const battle = (currentValue) => {
        const currentPlayerCard = characters.find(character => character.name === state.currentCard.player )
        const currentComputerCard = characters.find(character => character.name === state.currentCard.computer )
        // now we can check for the win condition. The dispatch payload needs only a boolean if the player wins or loses
        
        if (currentPlayerCard.[currentValue] > currentComputerCard.[currentValue]) {
            ...
            return `${currentPlayerCard.[currentValue]} : ${currentComputerCard.[currentValue]}`
        }
        if (currentPlayerCard.[currentValue] <= currentComputerCard.[currentValue]) {
            const newPlayerCards = [...state.cards.player]
